...
$app->match('/users/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $update_query = "INSERT INTO `users` (`username`, `password`, `email`) VALUES (?, ?, ?)";
            $app['db']->executeUpdate($update_query, array($data['username'], $data['password'], $data['email']));            
...
$app->match('/users/edit/{id}', function ($id) use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $update_query = "UPDATE `users` SET `username` = ?, `password` = ?, `email` = ? WHERE `user_id` = ?";
            $app['db']->executeUpdate($update_query, array($data['username'], $data['password'], $data['email'], $id));            
